...
public class JavaUnsafeResourceReallocation extends JavaDetector{
	private Neo4jDriver dbDriver;
	private static final String type = "不安全的资源释放：同一try语句中多个资源释放，这可能导致后面的释放不成功";
	private static final String defectPattern = "MATCH p=(e:Expression{expressionType:'MethodInvocation', methodName:'close'})<-[*]-(bin)<-[r:statementBody]-(t:Statement{statementType:'TryStatement'})<-[statementBody]-(bout:Statement{statementType:'Block'}) "
			+ "WHERE ALL(r IN relationships(p) WHERE NOT type(r)='methodBinding') "
			+ "WITH t AS try, COUNT(e) AS num "
			+ "WHERE num>1 "
			+ "RETURN try.belongTo, try.rowNo";

	public JavaUnsafeResourceReallocation() {
